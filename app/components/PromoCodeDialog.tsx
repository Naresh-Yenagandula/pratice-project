          ...
          <div className="flex items-center gap-3">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full rounded-md border border-gray-300 pl-10 pr-3 py-2 text-sm font-bold text-black placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-red-500"
              />
            </div>
            <button
              type="button"
              onClick={() => { updateTabData(activeTab, { promoCode: code.trim() }); onClose(); }}
              className="bg-red-600 text-white px-5 py-2 rounded-md text-sm font-medium hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-red-500"
            >Submit</button>
          </div>
          <hr />
          <p className="text-sm text-black">Maximize your savings with our exclusive promo code! / Shukran Id!</p>
        </div>
      </div>
    </div>
  );
